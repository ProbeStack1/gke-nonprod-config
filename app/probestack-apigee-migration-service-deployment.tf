resource "kubernetes_deployment_v1" "probestack_apigee_migration_service" {
  metadata {
    ...
    namespace = "dev"
    labels    = { app = "probestack-apigee-migration-service" }
  }
  spec {
    replicas = 1
    selector { match_labels = { app = "probestack-apigee-migration-service" } }
    template {
      metadata { labels = { app = "probestack-apigee-migration-service" } }
      spec {
